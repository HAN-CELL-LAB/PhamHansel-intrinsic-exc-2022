...
plot_fieldpairs = {'discrimination_accuracy', 'recognition_TPR', 'recognition_postradeoffs'};
title_name = struct; 
...
nearest_select_alpha = [0.4, 0.50, 0.6, 0.75, 0.85, 1];
...
fig_name = fullfile(fig_path, 'ttt');
...
cbar.Position = cbar.Position .* [1,1,1.2,0.7] + [0.05,0.05,0,0];
title(cbar, '$\alpha_W$', 'interpreter', 'latex');
set(cbar, cbar_props{:});

% fig_name = fullfile(fig_path, 't3');
% export_fig(fig_name,  '-r300', '-p0.02');
% pause(0.5); close;

%% Plot delta_theta_optim, TRP, TPR-FPR, discacc
title_name = struct; 
title_name.recognition_accuracy = ['recognition' newline 'TPR'];
title_name.recognition_postradeoffs = ['recognition' newline 'TPR - FPR'];
title_name.discrimination_accuracy = ['discrimination' newline 'accuracy'];
