...
debug_fig_path = 'figures/extra-s1/debug';
...
if ~exist(debug_fig_path, 'dir')
    mkdir(debug_fig_path);
end
%% Configs 
...
for i = 1:n_trace
    ...
    if ~isnan(debug_fig_path)
        ...
        fig_name = fullfile(debug_fig_path, regexprep(file_main_name, '.mat', sprintf('-trace%02d.png', i)));
        figure('units', 'normalized', 'position', [0,0,1,0.5], 'visible', 'off'); hold on;
        plot(t, Vm, '-k'); 
        plot(AP_properies.AP_time,AP_properies.AP_Vm, 'ro');
        plot(AP_properies.AP_thres_time,AP_properies.AP_thres_Vm, 'bo');
        xlim([50, 0.6*t(end)]); ylim([-85,50]); 
        despline; 
