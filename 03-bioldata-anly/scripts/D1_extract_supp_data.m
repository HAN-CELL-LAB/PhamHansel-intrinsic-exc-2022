clc; clear; close all;
run start_up.m
run D1_extract_supp_data_config.m
...
represent_struct.post = struct('portion', [0.2, 0.4, 0.9]);

%% Analysis table
% TODO: need to append to already processed 
% unproc_cells = ~info_summary_table.processed;
% selected_info_table = info_summary_table(unproc_cells,:);
selected_info_table = info_summary_table;
...
mat_file = fullfile(data_path, [cell_info_struct.full_ID '-basepost.mat']);
data = load(mat_file).recordings; 
is_base = contains({data.tag}, 'base'); 

[base_summary, base_repr] = process_data_per_file(data(is_base), config_struct, represent_struct.base, dry_run);
[post_summary, post_repr] = process_data_per_file(data(~is_base), config_struct, represent_struct.post, dry_run);
...
function [summary_struct, represent_traces] = process_data_per_file(...
    recordings, config_struct, represent_struct, dry_run)
if ~exist('dry_run', 'var')
    dry_run = false;
end
...
for i = 1:n_trace
    ...
    I_hold = trace_struct.Ihold;
    cell_info.Ihold = I_hold;
    ...
    summary_struct(i).I_hold = I_hold; 
